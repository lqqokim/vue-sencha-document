<template>
    <div class="center">
        <div class="left-btn" ref="leftBtn" :class="{ 'visible': isLeftBtnVisible, 'prevent': isLeftBtnDisable}" @click="onClickLeft"></div>
        <div class="right-btn" ref="rightBtn" :class="{ 'visible': isRightBtnVisible, 'prevent': isRightBtnDisable }" @click="onClickRight"></div>
        <div class="header" ref="header">
            <div class="tab-wrapper" ref="tabs">
                <div class="tab" v-for="(item, index) in tabs" :key="index" @click="onClickTab(index)" :class="{ 'active' : selectedTabIndex === index, 'close': item.isClose}">
    ...
    export default {
        ...
        methods: {
            ...
            onCloseTab(index) {
                ...
                this.selectedTabIndex = (index === this.tabs.length - 1) ? index - 1 : index + 1;
            ...
            onClickLeft() {
                ...
                const firstTabRef = tabsRef.children[0]; // 첫번째 tab
                let right = Number(this.$refs.tabs.style.left.split('p')[0]);
                tabsRef.style.left = right + 4 + 'px';
            ...
            onClickRight() {
                ...
                const lastTabRef = tabsRef.children[tabsChildren.length - 1]; // 마지막 tab
                const left = Number(tabsRef.style.left.split('p')[0]);
                tabsRef.style.left = left - 4 + 'px';
            ...
            checkButtonStatus() {
                ...
                tabsWidthSize >= this.$refs.header.offsetWidth && this.activeButtons();
            },
            addResizeEvent() {
                this.$nextTick(e => {
                    let timer;
            ...
            handleResize() {
                ...
                tabsWidthSize >= this.$refs.header.offsetWidth ? this.activeButtons() : this.inactiveButtons();
            },
        },
        mounted() {
